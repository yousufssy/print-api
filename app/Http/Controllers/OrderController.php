        ...
        if ($q) {
            $query->where(function ($query) use ($q) {
                $query->where('Customer', 'like', "%$q%")
                      // SQL Server does not allow LIKE on numeric columns directly.
                      ->orWhereRaw('CAST([Demand] AS NVARCHAR(50)) LIKE ?', ["%$q%"]);

                // Compare ID only when search term is numeric to avoid conversion errors.
                if (is_numeric($q)) {
